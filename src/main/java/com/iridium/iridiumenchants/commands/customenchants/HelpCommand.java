...
public class HelpCommand extends Command {
    ...
    public boolean execute(CommandSender sender, String[] arguments) {
        List<Command> availableCommands = IridiumEnchants.getInstance().getCommandManager().commands.stream()
        ...
        // Prepare the footer
        TextComponent footerText = new TextComponent(StringUtils.color(IridiumEnchants.getInstance().getMessages().helpCommandFooter
        ...
        // Send all messages
        sender.sendMessage(StringUtils.color(IridiumEnchants.getInstance().getMessages().helpCommandHeader));
        availableCommands.stream()
                .skip((page - 1) * 8L)
                .limit(8)
                .map(command -> StringUtils.color(IridiumEnchants.getInstance().getMessages().helpCommandMessage
    ...
    public List<String> onTabComplete(CommandSender commandSender, org.bukkit.command.Command cmd, String label, String[] args) {
        if (args.length == 2) {
            int availableCommandAmount = (int) IridiumEnchants.getInstance().getCommandManager().commands.stream()
